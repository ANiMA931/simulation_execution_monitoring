...
Network = ET.parse("E:\\code\\PycharmProjects\\simulation\\NetWorkXml\\MyCrowd_Network.xml")  # 读取MyCrowd_Network.xml文件
...
for i in range(10):
    for j in range(10):
        GroupXml[i*10+j] = 'MyCrowd_Primitive' + \
            str(i)+str(j)  # MyCrowd_Primitiveij来命名xml文件名
...
for k in range(100):
    ...
    node_model_information.attrib = {"version": '0.0',
                                     "modifyDate": '2019.04',
                                     "author": "MA", "comment": "No"}  # modelInformation设定属性version、modifyDate、author、comment
    ...
    aa ='E:\\code\\PycharmProjects\\simulation\\primitive\\'+ GroupXml[k]+'.xml'
    tree.write(aa)
# print(resetTime)# 瞅一眼最后能重复多少次，次数大约在sum_i(1/单位i被连接数+1/单位i主动连接数)之间
