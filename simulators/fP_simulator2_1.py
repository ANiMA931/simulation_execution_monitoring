...
def thread_main_loop_for_collective(collective_id: str):
    """
    集合型单元主循环
...
def special_loop_for_collective(group: int, collective_ids: list):
...
def transmit_message_for_collective(id_from_where: str, message: Message, id_to_where: str):
    ...
    if id_from_where in primitives[id_to_where].the_connectors.keys():
        primitives[id_to_where].msg_receive_pool.append([message.strength, deepcopy(message), id_from_where])
...
def make_decision(primitive_id: str, message: Message):
    ...
    for one_preference in local_preference:
        if message.area == one_preference.area:
            proj = projection(one_preference, message)
            break
    else:
        proj = 0
    if proj > 1:
...
def message_filtering(primitive_id: str, the_round: int):
    """
    ...
    if the_round % 95 == 0:
        for s_msg in primitives[primitive_id].msg_storage_pool:
            s_msg[0] *= forget_rate
            s_msg[1].strength = s_msg[0]
...
if __name__ == '__main__':
    ...
    collective_generation = [220,221,222,223,224,
                             476,477,478,479,480,
                             767,768,769,790,791]
    special_group = [2, 4, 6]
    all_round = 1000
    ...
    # 生成excel表格
    wb = xlsxwriter.Workbook('simu_rec_table2.xlsx')
