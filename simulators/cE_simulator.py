...
def re_simulate(units, advisors, monitors, now_round, success_rate_list, record_path):
    ...
    for one_unit in units.keys():
        ectypal_advisors = {}
        ectypal_monitors = {}
        for adv in units[one_unit].effector['cE_advisors'].keys():
...
def make_round_record(units: dict, the_round, success_rate_list, record_path):
    ...
    for one_unit in units.keys():
        ...
        # 对于每个与该unit连接的建议者
        for temp_advisor in units[one_unit].effector['cE_advisors'].items():
            # 初始化advisor标签
        ...
        # 对于每一个与unit相连的监控者
        for temp_monitor in units[one_unit].executor['cE_monitors']:
            # 初始化monitor标签
...
def main():
    ...
    for one_unit in units.keys():
        for one_advisor in units[one_unit].effector['cE_advisors'].keys():
            units_advisors_relationship[one_unit][one_advisor] = units[one_unit].effector['cE_advisors'][one_advisor]
    ...
    for one_advisor in advisors.keys():
        for one_unit in advisors[one_advisor].unitList['cE_units'].keys():
            advisors_units_relationship[one_advisor][one_unit] = advisors[one_advisor].unitList['cE_units'][one_unit]
