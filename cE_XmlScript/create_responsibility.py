...
class responsibility_creator:
    ...
    def create_responsibility(self):
        ...
        '''for i in range(20):
            mntrdom = read_xml(
                "E:\\code\\PycharmProjects\\simulation\\cE_monitors\\" + "MyCrowd_monitor" + str(i).zfill(2) + ".xml")
            root = mntrdom.documentElement
            monitoring = root.getElementsByTagName('monitoring')[0]
            shuffle(b)
            ...
            for j in range(1, responsible_count):
                v += "|" + str(b[j])
            monitoring.setAttribute('value', v)
            write_xml("E:\\code\\PycharmProjects\\simulation\\cE_monitors\\" + "MyCrowd_monitor" + str(i).zfill(
                2) + ".xml", mntrdom)'''
